...
import { Observable, of, switchMap, take, tap, Subscription } from 'rxjs';
...
export class TradeFormComponent implements OnInit, OnDestroy {
  ...
  isBuy = false;
  isSell = false;
  private orderChangeSub?: Subscription;
  ...
  registerOrderChange(): void {
    this.orderChangeSub = this.tradeForm.get('order')?.valueChanges.subscribe((order) => {
  ...
  ngOnDestroy(): void {
    this.orderChangeSub?.unsubscribe();
  }
